...
public class EntityLoot implements Consumer<BiConsumer<ResourceLocation, LootTable.Builder>>
{
    ...
    {
        return LootTable.lootTable().withPool(LootPool.lootPool().setRolls(ConstantValue.exactly(1.0F)).add(LootItem.lootTableItem(pWool))).withPool(LootPool.lootPool().setRolls(ConstantValue.exactly(1.0F)).add(LootTableReference.lootTableReference(EntityType.SHEEP.getDefaultLootTable())));
    }
    ...
    private void add(EntityType<?> pId, LootTable.Builder pTable)
    {
        this.add(pId.getDefaultLootTable(), pTable);
    }

    private void add(ResourceLocation pId, LootTable.Builder pTable)
    {
        this.map.put(pId, pTable);
    }
}
